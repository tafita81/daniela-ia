// @ts-nocheck
// V19 — GPT-4.1-mini default + schema correto Supabase (cache_key/expires_at) + vision + arquivos
...
async function execTool(name,args){
  try{
    ...
    if(name==='health_check'){
      return JSON.stringify({openai:!!OAI,groq:!!GK,gemini:!!GEK,supabase:!!(SBK||SBA)});
    }
...
async function callGroq(messages,tools){
  const cleanMsgs=messages.map(m=>{
    if(Array.isArray(m.content))return{...m,content:m.content.filter(c=>c.type==='text').map(c=>c.text).join(' ')||'(imagem)'};
    return m;
  });
...
// ── SUPABASE — schema correto: cache_key + value + expires_at ────────────
const FAR=()=>new Date(Date.now()+365*24*3600*1000).toISOString();
async function sbSet(key,value){
  const k=SBK||SBA;if(!k)return false;
  try{
    const r=await fetch(`${SBU}/rest/v1/ia_cache?cache_key=eq.${encodeURIComponent(key)}`,
      {method:'DELETE',headers:{apikey:k,Authorization:`Bearer ${k}`}});
    const r2=await fetch(`${SBU}/rest/v1/ia_cache`,{
      method:'POST',
      headers:{apikey:k,Authorization:`Bearer ${k}`,'Content-Type':'application/json','Prefer':'return=minimal'},
      body:JSON.stringify({cache_key:key,value:typeof value==='string'?value:JSON.stringify(value),expires_at:FAR()})});
    return r2.ok;
  }catch{return false;}
}
async function sbGet(key){
  ...
  try{
    const r=await fetch(`${SBU}/rest/v1/ia_cache?cache_key=eq.${encodeURIComponent(key)}&select=value`,{headers:{apikey:k,Authorization:`Bearer ${k}`}});
    if(!r.ok)return null;
    const d=await r.json();
...
async function sbList(prefix,limit=50){
  ...
  try{
    const r=await fetch(`${SBU}/rest/v1/ia_cache?cache_key=like.${encodeURIComponent(prefix)}*&select=cache_key,value,expires_at&order=expires_at.desc&limit=${limit}`,
      {headers:{apikey:k,Authorization:`Bearer ${k}`}});
    if(!r.ok)return[];
    return(await r.json()).map(r=>{let v=r.value;try{v=JSON.parse(v);}catch{}return{key:r.cache_key,value:v,updated_at:r.expires_at};});
  }catch{return[];}
}
async function sbDelete(key){
  const k=SBK||SBA;if(!k)return false;
  try{
    const r=await fetch(`${SBU}/rest/v1/ia_cache?cache_key=eq.${encodeURIComponent(key)}`,{method:'DELETE',headers:{apikey:k,Authorization:`Bearer ${k}`}});
    return r.ok;
  }catch{return false;}
}
...
⚠️ REGRAS ABSOLUTAS:
1. Quando o usuário pedir AÇÕES ("configure", "deploy", "verifique", "edite", "liste", "analise", "descubra"), VOCÊ EXECUTA usando tools.
...
5. Quando receber imagem, descreva o que vê. Quando receber arquivo (PDF/ZIP/DOCX/código), processe o conteúdo passado no contexto.
...
export async function POST(req){
  try{
    ...
    if(body._action==='session_save'&&body._session_id){
      ...
      const ok=await sbSet(key,{title:body._title||'Chat',msgs:body.messages||[],updated_at:new Date().toISOString()});
      return NextResponse.json({ok,saved:key});
    }
    if(body._action==='sessions_list'){
      const list=await sbList(`chat:${body._user_id||'anon'}:`,50);
      return NextResponse.json({ok:true,sessions:list.map(s=>({id:s.key.split(':').pop(),title:s.value?.title||'Chat',updated_at:s.value?.updated_at||s.updated_at,msg_count:s.value?.msgs?.length||0}))});
    }
    ...
    if(body.fileContent&&body.fileName){
      ...
      let note='';
      if(fname.endsWith('.zip'))note=`[📦 ZIP: ${body.fileName} ${sz} — não consigo extrair ZIP nativamente, mas posso analisar conteúdo descrito]`;
...
export async function GET(){
  return NextResponse.json({
    ok:true,
    daniela:'V19 — GPT-4.1-mini default + tools + vision + arquivos + sessões corrigidas',
    ...
    schema:'cache_key + value + expires_at',
